    ...
    <div id='kouta' class="chatboxcontent" style="display : none;">
        ...
        if (isset($session_data_user['onlineUsers'])) {
            ...
            foreach ($session_data_user['onlineUsers'] as $res) {
                if ($session_data_user['user_name'] != $res->user_name) { ?>
                    <li>
                        <a style="line-height: 32px; cursor: pointer; text-decoration: none;" href="javascript:void(0)" onClick="javascript:chatWith('<?php echo $res->user_name; ?>');" rel="ignore">
                            <div style="padding-left: 1px;position: relative;">
                                <div style="float: left; height: 12px; position: relative;  width: 55px;">
                                    ...
                                    foreach ($session_data_user['chatPhotos'] as $row1):
                                        if($res->user_name===$row1['user_name']){ ?>
                                            <img id="image<?php echo $res->username; ?>" src="<?php echo base_url(); ?>mboufos/assets/uploads/<?php echo $row1['profileImage']; ?>" width="32" height="32" alt="" class="img">
                                <?php   }
                                    endforeach;?>
                                </div>
                                <div style="overflow: hidden; padding-left: 8px; text-overflow: ellipsis; white-space: nowrap; padding-top: 6px;">
                                    ...
                                    <div style="float: right; margin: 0 1px 0 4px; text-align: right;">
                                        <div class="_568z">
                                            <span id="<?php echo $res->user_name; ?>" style="background: <?php if ($res->status == 1) echo 'rgb(66, 183, 42)';  else echo 'red'; ?>; border-radius: 50%;display: inline-block;height: 6px;margin-left: 4px;width: 6px;"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
