...
class ChatController extends CI_Controller {
 	public function __construct()
        {
				...
				$this->load->model(['ChatModel','Product_model']);
	...
	public function index(){
		...
		//if($this->session->userdata['Admin']['role'] == 'Client_cs'){
			$list = $this->Product_model->EmpList();
			$data['strTitle']='All Employees';
			$data['strsubTitle']='Employees';
			$data['chatTitle']='Select Employee with Chat';
		//}//else{
	...
	public function get_chat_history_by_vendor(){
		$receiver_id = $this->$this->input->get('receiver_id');
		...
		foreach($history as $chat){  //:
			...
			$userName = $this->Product_model->GetName($chat['sender_id']);
