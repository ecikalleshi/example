...
public function chatswitch()
 
  	{
		...
		//?????????????????
  		...
  		case 'chatheartbeat':
  			$this->chatheartbeat($session_data_user['user_name']);
  		case 'sendchat':
  			$this->sendChat($session_data_user['user_name']);
  		case 'closechat':
  			$this->closeChat();
  		case 'startchatsession':
  			$this->startChatSession($session_data_user['user_name']);
  		default:
  			break;
  		endswitch;
  	}
  		...
  		if (!empty($query[0])) {
  			foreach($query[0] as $chat):
  				...
  				foreach($query[1] as $row1):
  					if ($chat['from'] == $row1['user_name']) { // check for the photo name
  						$userPhoto = $row1['profileImage'];
  					}
  				endforeach;
